...
(async () => {
    ...
    // 2. Setup links.txt if missing (Matches your default text)
    if (!fs.existsSync(IN_DIR)) {
        const defaultText =
            "Paste the theatre links under here, separated by new line\n" +
            "https://www.more.com/gr-el/tickets/theater/example-play/"
    ...
    for (const url of links) {
        ...
        if (!page.json) {
            try {
                ...
                // Your random sleep logic (2 to 5 seconds roughly)
        ...
        // Process Logic
        try {
            // Replicating your Name extraction: page["json"]["plays"][0]["play-title"]
            // Added optional chaining (?.) so it doesn't crash if 'plays' is missing
            ...
            console.error(`Error processing data for ${engName}: ${e.message}`)
        }
    }
